...
void shortest_path(GraphType* g, int start) { // start_vertex 번호 받기
	...
	for (int i = 0; i < g->n; i++) {
		...
		for (w = 0; w < g->n; w++) {
			if (!found[w]) { // !found 주의...
				if (distance[u] + find_weight(g, u, w) < distance[w]) //더 짧을 경우
					distance[w] = distance[u] + find_weight(g, u, w); // distance[w]값 갱신
			}
		}
	}
}
int main() {
	...
	shortest_path(g, start); //0번 vertex에서 모든 vertex까지의 shortest path
