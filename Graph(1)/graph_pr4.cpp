...
void dfs_list(GraphType* g, int value) {
	GraphNode* w;
	visited[value] = true; // dfs 시작된 vertex는 방문한 것으로 표시
	printf("vertex%d ", value);
	for (w = g->adj_list[value]; w; w = w->link) {
		if (!visited[w->vertex]) { dfs_list(g, w->vertex); } //true가 아니면, 거기서부터 다시 재귀 시작
	}
}
int main() {
	GraphType* g;
